...
if(!empty($file_name))
{
  ...
  // if everything is ok, try to upload file
      ...
      if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
          ...
          $location = "uploads/services/".$fileNewName; 
          mysqli_query($conn,"UPDATE services 
                      set judul = '$judul', deskripsi = '$desc', publish = '$publish',
                      url_gambar = '$location',updated_at = now()
                      where id = $id");
          $conn->close();

          echo "<script>alert('Edit $judul berhasil!')
          location.replace('../../services')</script>";
      } else {
...
else 
{
  ...
  $sql = "UPDATE services 
          SET judul='$judul', deskripsi = '$desc', publish = '$publish', updated_at = now()
          WHERE id=$id";
